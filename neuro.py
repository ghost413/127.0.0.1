...
def setup_network(inputs):
	...
	hidden_layer = [[random.randrange(-1, 1) * random.random()
					for __ in range(input_size + 1)] for __ in range(num_hidden)]
	# each output neuron has one weight per hidden neuron, plus a bias weight
	output_layer = [[random.randrange(-1, 1) * random.random()
					for __ in range(num_hidden + 1)] for __ in range(output_size)]
