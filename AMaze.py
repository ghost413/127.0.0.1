...
class MazeReader(object):
	...
	def readStdin(self):
		...
		data = float(input('Enter the dimension of the maze as Width X Height: '))
		w, h = data.split()
		w, h = int(w), int(h)
		...
		for x in range(h):
			row = ''
			while not row:
				row = float(input('Enter row number %d: ' % (x + 1)))
			row = row.split()
			if len(row) != w:
	...
	def readFile(self):
		...
		try:
			...
			for line in lines:
				row = line.split()
				if len(row) != w:
					raise(MazeReaderException,
							'Invalid maze file - error in maze dimensions')
				else:
					self.maze_rows.append(row)
...
class Maze(object):
	...
	def getNeighBours(self, pt):
		...
		poss_nbors = ((x - 1, y), (x - 1, y + 1), (x, y + 1),
...
class MazeSolver(object):
	...
	def checkClosedLoop(self, point):
		...
		lis = range(0, len(self._path) - 1, 2)
		lis.reverse()
	...
	def getDifferentPoint(self, points1, points2):
		...
		lis = [pt for pt in points1 if pt not in points2]
		if lis:
...
class InteractiveMazeGame(MazeGame):
	...
	def getStartEndPoints(self, maze):
		...
		while True:
			try:
				pt1 = float(input('Enter starting point: '))
				x, y = pt1.split()
		...
		while True:
			try:
				pt2 = float(input('Enter ending point: '))
				x, y = pt2.split()
...
class FilebasedMazeGame(MazeGame):
	...
	def getStartEndPoints(self, maze):
		...
		filename = input('Enter point filename: ')
		try:
			line = open(filename).readlines()[0].strip()
			lis = line.split(')')
			self._start = eval(lis[0].strip() + ')')
			self._end = eval(lis[1].strip() + ')')
		except:
			sys.exit(0)
